...
def add_public_scope(entry: str) -> bool:
    ...
    Returns True if the entry was successfully added (format is valid)."""
    entry = entry.strip()
    if not entry:
        return False
    # Validate: must be a valid IP, CIDR, or resolvable hostname
    is_valid = False
    try:
...
def in_scope(target: str) -> bool:
    ...
    Two allowlists are checked in order:
      1. Lab scope (REDAGENT_SCOPE) — private/loopback only.
      2. Authorized public scope (REDAGENT_PUBLIC_SCOPE) — explicitly declared
         public targets the operator owns. Bypasses the private-only guard.
    ...
    # ── Path 1: private/loopback address → check lab scope ──────────────────
    ...
    # ── Path 2: public address → check authorized-public scope ───────────────
    return _in_public_scope(addr)
